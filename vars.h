...
#define ESC_CMD 	0x7d
#define SESRST_CMD	0x55//session reset
#define SESEND_CMD	0x56//for compatibility
#define BSYNC		0x7E//for compatibility
...
//Ќа каждую прин€тую от компьютера команду, или проще сказать на каждый прин€тый байт,
// клавиатура/мышь должны об€зательно ответить одним из следующих байтов:
#define ACKNOWLEDGE 0xFA
//Ц подтверждение об успешном приеме;
#define RESEND 0xFE

#define ERROR 0xFF

//ƒл€ клавиатуры компьютер может послать следующие команды:
...
//Ц это тоже двухбайтова€ команда. ѕосле этой команды следует параметр определ€ющий частоту
// повтора кодов при нажатой клавише и интервал времени между нажатием и началом повторов.
// ѕараметр байт typematic выгл€дит следующим образом:
// 0  	 d6  	 d5  	 t4  	 t3  	 t2  	 t1  	 t0 
//Ѕит 7  - всегда ноль.
//Ѕиты d5 and d6 определ€ют задержку (Delay) от времени нажати€, когда посылаетс€ первый код,
// до момента когда начинаютс€ повторы кодов, если удерживать клавишу. 
//«адержку можно вычислить по формуле (1+typematic[6:5])*250 миллисекунд.(250-1000ms)
//Ѕиты t4, t3, t2, t1, t0 определ€ют частоту повторов кодов при удерживании клавиши нажатой.
// ѕериод повторов можно вычислить по формуле:
//Period = (8 + typematic[2:0]) * (2^typematic[4:3]) * 0.00417 секунд.(33-500ms)

#define KEYBOARD_RESET 0xFF
// Ц получа€ эту команду клавиатура отвечает, как обычно, 0xFA, а затем, сбрасываетс€ и посылает
// в ответ байт
...
//ѕосле подачи напр€жени€ питани€ клавиатура посылает компьютеру код KEYBOARD_COMPLETE_SUCCESS
 //и немедленно готова к работе.
//ѕо умолчанию клавиатура посылает на нажатие один байт-код, а на отжатие клавиши два байта.
// ѕервый байт в кодах "отпускани€ клавиши" Ц это префикс отжати€
