...
#define SendKeyRet(x) {SetSession(KEY_CMD);USART_PutChar(x);}
...
#define SendKeyData(x) {SendESCifNeed(x);USART_PutChar(x);}
...
//******** void rxUdata(void)***************
		{
			...
			case SESRST_CMD://session reset
				...
				break;
			case BSYNC:
				wdt_enable(WDTO_120MS);//for debug
				while(1);
